...
from typing import Dict, Generic, ItemsView, List, NamedTuple, Optional, Set, Type, TypeVar, Callable, Iterable, Union, overload
...
def _validate_enum(enum: Type[IntEnum]) -> Type[IntEnum]:
    if enum is None:
        raise ValueError("None provided, expected IntEnum")
    for i, val in enumerate(enum):
        if i != val:
            raise ValueError(f"Value {val} in {enum} breaks sequence: {val.value} != {i}")
    return enum
...
class FsmDefinition(Generic[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    def __init__(self, states: Type[StatesEnum], inputs: Type[InputsEnum], outputs: Type[OutputsEnum] = NoOutputs,
                 ...
                 auto_finalize: bool = True) -> None:

        self._states_type: Type[StatesEnum] = _validate_enum(states)

        self._inputs_type: Type[InputsEnum] = _validate_enum(inputs)

        self._outputs_type: Type[OutputsEnum] = _validate_enum(outputs)
    ...
    def __setitem__(self, key, value):
        #TODO: implmement this for state builders
        pass
    ...
    def add_state_input(self, state: StatesEnum,
                        input: Union[InputsEnum, Iterable[InputsEnum]]) -> FsmDefinition[StatesEnum, InputsEnum, OutputsEnum, DataType]:
        if self._ready is True:
            raise DefinitionError(f"Cannot update a finalized Fsm Definition {self}")
    ...
    def remove_state_input(self, state: StatesEnum, input: Union[InputsEnum, Iterable[InputsEnum]]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        if self._ready is True:
            raise DefinitionError(f"Cannot update a finalized Fsm Definition {self}")
    ...
    def set_output(self, output: OutputsEnum,
                   callbacks: Iterable[Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
    ...
    @overload
    def add_output_callback(self, output: OutputsEnum,
                            callback: Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        ...

    @overload
    def add_output_callback(self, output: OutputsEnum,
                            callback: Iterable[Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        ...

    def add_output_callback(self, output: OutputsEnum, callback: Union[
        Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None], Iterable[
            Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]]]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
    ...
    @overload
    def remove_output_callback(self, output: OutputsEnum,
                               callback: Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        ...
    ...
    @overload
    def remove_output_callback(self, output: OutputsEnum,
                               callback: Iterable[Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]]) -> \
            FsmDefinition[StatesEnum, InputsEnum, OutputsEnum, DataType]:
        ...

    def remove_output_callback(self, output: OutputsEnum, callback: Union[
        Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None], Iterable[
            Callable[[FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]], None]]]) -> FsmDefinition[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        output_callbacks = self._output_callbacks[output]
    ...
    def finalize(self) -> None:
        ...
        if len(names ^ set(self._inputs_type)) > 0:
            ...
            raise DefinitionError(
                f"Cannot finalize Fsm Definition {self}, as built Inputs {names} do not coinside with defined Inputs {self._inputs_type}")
    ...
    def _trigger_state(self, fsm_implementation: FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType],
                       state: Optional[StatesEnum]) -> None:
        if state is not None:
            fsm_implementation._state = state

    def _trigger_input(self, fsm_implementation: FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType], input: InputsEnum) -> None:
        # TODO: Handle busy correctly, i.e. add queueable property or immediate or throw
        if fsm_implementation._busy:
            raise TriggerError()

        if input not in self._inputs_in_state[fsm_implementation._state]:
            raise TriggerError()

        with fsm_implementation as impl:
            old_state = fsm_implementation._state

            expression_runner = self._input_expression_runner[input]
            # if expression_runner is None:
            #     raise RuntimeError()
            with FsmController(self, impl) as fsm_controller:
                expression_runner(impl._data, fsm_controller)  # type:ignore


    def implement(self, initial_state: Optional[StatesEnum] = None,
                  name: Optional[str] = None) -> FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]:
...
class FsmImplementation(Generic[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        # TODO: implement reaction to exceptions
        self._busy = False
    ...
    @overload
    def add_output_callback(self, output: OutputsEnum, callback: Iterable[Callable]) -> FsmImplementation[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        ...

    def add_output_callback(self, output: OutputsEnum, callback: Union[Callable, Iterable[Callable]]) -> FsmImplementation[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        output_callbacks = self._output_callbacks[output]
        if isinstance(callback, Iterable):
            output_callbacks.update(callback)
    ...
    @overload
    def remove_output_callback(self, output: OutputsEnum, callback: Iterable[Callable]) -> FsmImplementation[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        ...

    def remove_output_callback(self, output: OutputsEnum, callback: Union[Callable, Iterable[Callable]]) -> FsmImplementation[
        StatesEnum, InputsEnum, OutputsEnum, DataType]:
        output_callbacks = self._output_callbacks[output]
        if isinstance(callback, Iterable):
            output_callbacks.difference_update(callback)
...
class ChainExpression(Expression[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    ...
    def Action(self, function: Callable[[DataType], None]) -> ChainExpression[StatesEnum, InputsEnum, OutputsEnum, DataType]:
        self._extend_tail(Action(function))
    ...
    def Condition(self, condition: Callable[[DataType], bool],
                  next_true: Expression[StatesEnum, InputsEnum, OutputsEnum, DataType],
                  next_false: Expression[StatesEnum, InputsEnum, OutputsEnum, DataType]
                  ) -> ChainExpression[StatesEnum, InputsEnum, OutputsEnum, DataType]:
...
class Action(ChainExpression[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    ...
    def __init__(self, function: Callable[[DataType], None]) -> None:
        super().__init__(expression_type=ExpressionType.Action)

        self.callable: Callable[[DataType], None] = function
        self.next: Optional[Expression[StatesEnum, InputsEnum, OutputsEnum, DataType]] = None
...
class Condition(ChainExpression[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    def __init__(self, condition: Callable[[DataType], bool], next_true: Expression[StatesEnum, InputsEnum, OutputsEnum, DataType],
                 next_false: Expression[StatesEnum, InputsEnum, OutputsEnum, DataType]) -> None:
...
class FsmController(Generic[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    def __init__(self, fsm_definition: FsmDefinition[StatesEnum, InputsEnum, OutputsEnum, DataType],
                 fsm_implementation: FsmImplementation[StatesEnum, InputsEnum, OutputsEnum, DataType]) -> None:
        self._fsm_definition: FsmDefinition[StatesEnum, InputsEnum, OutputsEnum, DataType] = fsm_definition
...
class CallableExpressionBuilder(ExpressionBuilder[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    def __init__(self, states: Type[StatesEnum], inputs: Type[InputsEnum],
                 outputs: Type[OutputsEnum], data: Optional[Type[DataType]],
                 named_expressions: Dict[InputsEnum, Expression]) -> None:
    ...
    def _connect_callable_expressions(self, previous_callable_expression_point: CallableExpressionPoint,
                                      current_callable_expression: CallableExpression):
        if previous_callable_expression_point.previous_callable_expression is not None:
            if isinstance(previous_callable_expression_point.previous_callable_expression, (CallableAction, CallableOutput, CallableState)):
                ...
                raise ExpressionError(
                    f"Cannot assign next expression to callable expression {previous_callable_expression_point.previous_callable_expression}.")
        else:
    ...
    def build(self):
        try:
            for name, head_expression in self._named_expressions.items():
                ...
                visited_expressions = {head_expression}
...
class CallableExpression(ABC, Generic[StatesEnum, InputsEnum, OutputsEnum, DataType]):
    @abstractmethod
    def __call__(self, data: DataType, fsm_controller: FsmController[StatesEnum, InputsEnum, OutputsEnum, DataType]) -> Optional[
        CallableExpression[StatesEnum, InputsEnum, OutputsEnum, DataType]]:
        pass
...
#FIXME: panda_definition.set_input(PandaState.Sleep, PandaInput.Rest, Action(...))

# sub_builder_sleep = panda_definition[PandaState.Sleep]
# sub_builder_sleep[PandaInput.Rest] = Action(partial(action_print, msg="Rest"))
#                .Condition(
#                    happiness_condition
#                    ,
...
with panda_definition[PandaState.Sleep] as state_builder:
    ...

with panda_definition[PandaState.Sleep, PandaInput.Rest] as input_builder:
    ...
...
panda_definition \
    ...
    .set_input(PandaInput.Feed,
               ...
               .Condition(
                   random_condition,
                   Action(partial(action_print, msg="Random True")).State(None),
                   Action(partial(action_print, msg="Random False")).State(PandaState.Happy)
               )
               ) \
